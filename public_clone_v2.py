...
╰──────────────────────────────────────────────╯
         {sir} AUTHOR : MD Sabbir Hossain Rafi ✅{x}''')
#--------------------[ IT'Z S H R✅]--------------#
def login():
	try:
...
#-----------------[ RAFU😘-CRACK ]-----------------#
def result():
	print(f'└──[1] RAFU😍 {h}OK{x} ANDA ')
	print(f'└──[2] RAFU😭 {k}CP{x} ANDA ')
	print('└──[3] KEMBALI	')
	kz = input(f'\n├└──[Select✅ : ')
	if kz in ['2']:
		try:vin = os.listdir('CP')
			...
			back()
		if len(vin)==0:
			print('└──ANDA TIDAK MEMILIKI RAFU CP ')
			time.sleep(2)
			back()
		else:
			cih = 0
...
def dump_massal():
	...
	for met in range(jum):
		yz+=1
		kl = input(f'└──[{sir}INPUT ID{x} '+str(yz)+' : ')
		print('──────────────────────────────')
		uid.append(kl)
	for userr in uid:
		try:
...
#-------------[ PENGATURAN-ID ]---------------#
def setting():
	print(f'└──[1] {k}OLD Crack{x} ')
	print(f'└──[2] {h}NEWCrack{x} ')
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('───────────────────────────────────────────────────')
	_ilman_ = input('└──[Show Facebook Cookie💋 [ Y/T ] : ')
	if _ilman_ in ['']:
		print('└──PILIH YANG BENAR ')
		back()
	elif _ilman_ in ['y','Y']:
		...
		taplikasi.append('no')
	pwplus=input('└──[Use Manual Or Default password🤷‍♂️ [ Y/T ] : ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	cetak(nel('\t[green]Please Wait Procesing is Started'😊))
	print(f'└──[RAFU {h}OK{x} RAFU'S CLONE ID : {h}OK/%s {x}'%(okc))
	print(f'└──[RAFU {k}CP{x} RAFU'S CLONE ID : {k}CP/%s {x}'%(cpc))
	cetak(nel('\t[green]Turn on Airplane mood after 500 cross counting🕦 [white]10 DETIK'))
	print('|')
	print('|')
	...
	print(f'[{b}•{x}]{h} RAFU OK😍 : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{k} RAFUCP😭 : {k}%s{x} '%(cp))
#--------------------[ METODE-B-API ]-----------------#
def crack(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r└──{sir}[RAFU]{x} 😁 {P}{b}{loop}{P}/{u}{len(id)}{P} OK {P}{H}{ok}{P} CP {P}{k}{cp}{x} : {bo}{'{:.0%}'.format(loop/float(len(id)))}{P}  "), 
	sys.stdout.flush()
...
if __name__=='__main__':
	...
	ilman(f'\n\t{x}{sir} └── MD Sabbir Hossain Rafi ✅ :> {x}')
	time.sleep(3)
	login()
#IlmanRamdhaniRahman
